...
public class AddSubTransportServlet extends HttpServlet {
	...
	private SaveDivStoreDao saveDivStoreDao = new SaveDivStoreDaoImpl();
	...
	protected void doGet(HttpServletRequest request,
		...
		request.getRequestDispatcher("/jd/jd_zengjiadaiyundan.jsp").forward(
				request, response);
	}

	protected void doPost(HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException {
		System.out.println("准备处理代运配送单");
		...
		//下面是记录独立库存单
		saveDivStoreDao.addSaveDivStore(orderId,goodName,goodQuantity,Creater,1,sendtime);
