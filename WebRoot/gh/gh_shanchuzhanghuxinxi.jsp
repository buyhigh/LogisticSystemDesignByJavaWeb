...
<div class="page-container">
	...
	<form class="form-horizontal" role="form" name="myForm" action="DelUser" method="post" id="form1" style="margin-left: 300px;margin-right: 450px;color:black">
	<div class="form-group">
		<label class="col-sm-2 control-label">删除用户id</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" 
				   placeholder="请输入用户id" name="deluserId" style="color:black" autocomplete="off">
		</div>
	</div>
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
			<button type="submit" class="btn btn-default">删除账户</button>
		</div>
	</div>
</form>
	<table class="table table-striped"
			style="margin-left: 400px; margin-right: 200px; width: 600px">
			...
			<tbody>
				<c:forEach items="${requestScope.listCustomer }" var="customerentry"
					varStatus="s">
					<tr>
						<td>${customerentry.userId}</td>
						<td>${customerentry.userName}</td>
						<td>${customerentry.userType}</td>
					</tr>
				</c:forEach>
				<c:forEach items="${requestScope.listStorer }" var="storerentry"
					varStatus="s">
					<tr>
						<td>${storerentry.userId}</td>
						<td>${storerentry.userName}</td>
						<td>${storerentry.userType}</td>
					</tr>
				</c:forEach>
				<c:forEach items="${requestScope.listTransporter }" var="transporterentry"
					varStatus="s">
					<tr>
						<td>${transporterentry.userId}</td>
						<td>${transporterentry.userName}</td>
						<td>${transporterentry.userType}</td>
					</tr>
				</c:forEach>
				<c:forEach items="${requestScope.listMarketer }" var="marketerentry"
					varStatus="s">
					<tr>
						<td>${marketerentry.userId}</td>
						<td>${marketerentry.userName}</td>
						<td>${marketerentry.userType}</td>
					</tr>
				</c:forEach>
