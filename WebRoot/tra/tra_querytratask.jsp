...
<body>
	...
	<form id="form1" name="myForm" action="querytratask" method="post">
		<input type="hidden" name="curState" value="${requestScope.curState }"
			id="curSelState" />
		<div class="wl_content_nav">查询运输任务单</div>
		<table class="wl_table_query" cellspacing="0">
			<tr class="first">
				<th width="100">运输任务单状态</th>
				...
				<td width="460"><input type="submit" value="查询" /></td>
			</tr>
		</table>
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listTransferTaskDto }"
					var="transferTaskDto">
					<tr>
		...
		<div class="wl_page">
			...
			<c:if test="${requestScope.maxPage > 1 && requestScope.curPage == 1}">
                        	...
                        	[<a disabled="disabled">上一页</a>]
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
			<c:if
				test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=1">首页</a>]
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="querytratask?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
