...
<div class="page-container">
	...
	<form id="form1" name="myForm" action="querysendtratask" method="post">
            			<input type="hidden" name="curState" value="${requestScope.curState }" id="curSelState"/>
                		<div class="wl_content_nav">查询派车任务单</div>
		...
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listSendTraTaskDto }" var="sendTraTaskDto">
                            <tr>
                            	<td>${sendTraTaskDto.transferPlaceStr }</td>
                                <td>${sendTraTaskDto.transferModelStr }</td>
                                <td>${sendTraTaskDto.transferToolStr }</td>
                                <td>${sendTraTaskDto.requestNum }</td>
                                <td>${sendTraTaskDto.stateStr }</td>
                            </tr>
                            </c:forEach>
			</tbody>
		</table>
		<div class="wl_page">
                        	...
                        	[<a disabled = "disabled">首页</a>]
                        	[<a disabled = "disabled">上一页</a>]
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
                        	<c:if test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=1">首页</a>]
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="querysendtratask?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
    ...
    <script type="text/javascript">
    	...
    	window.onload = function(){
    		var curSelStateValue = $("curSelState").value;
    		var stateOptions = $("selState").options;
    		for(var i = 0 ; i < stateOptions.length ; i++){
    			if(curSelStateValue == stateOptions[i].value){
