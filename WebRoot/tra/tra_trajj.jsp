...
<style type="text/css">
	...
	.wl_table td.lastTd{text-align:left;}
...
<div class="page-container">
	...
	<form id="form1" name="myForm" action="trajj" method="post">
            			<input type="hidden" name="curDispUserId" value="${requestScope.curDispUserId }" id="hdnDisp"/>
            			<input type="hidden" name="flag" value="" id="f"/>
                		<div class="wl_content_nav">运输交接</div>
              		<table class="wl_table_query" cellspacing="0">
                        	<tr class="first">
                            	<th width="70">配送员编号</th>
                                <td width="120">
                                	<input type="text" name="dispUserId" id="stateDisp"/>
                                </td>
                                <td width="330">
		...
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listTransferTaskDto }" var="transferTaskDto">
                           <tr>
                           		<td><input type="checkbox" name="chkItem" value="${transferTaskDto.transferTaskId }" ${transferTaskDto.stateId == 6 ? 'disabled' : ''}/></td>
    ...
    <script type="text/javascript">
    	...
    	window.onload = function(){
    		var hdnValue = $("hdnDisp").value;
    		if(hdnValue != "-1"){
    			$("stateDisp").value = hdnValue;
    		}else{
    			$("stateDisp").value = "";
    		}
