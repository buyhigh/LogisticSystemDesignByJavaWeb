...
<div class="page-container">
	...
	<form id="form1" name="myForm" action="CarManager" method="post">
		...
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listTransferToolDto }"
					var="transferToolDto">
					<tr>
						<td>${transferToolDto.transferId }</td>
		...
		<div class="wl_page">
			...
			<c:if test="${requestScope.maxPage > 1 && requestScope.curPage == 1}">
                        	...
                        	[<a disabled="disabled">上一页</a>]
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
			<c:if
				test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=1">首页</a>]
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="CarManager?curSelPlace=${requestScope.curTransferPlace }&curSelModel=${requestScope.curTransferModel}&pageNum=${requestScope.maxPage }">末页</a>]
	...
	<form action="CarStateChange" method="post" role="form" style="margin-left: 600px;width: 400px">
        <div class="form-group"">
            <label for="name">选择车辆列表</label>
            <select class="form-control" name="selectcar">
            <c:forEach items="${requestScope.listTransferToolDto }" var="transferToolDto">
                <option>${transferToolDto.transferId}</option>
            </c:forEach>
            </select>
            ...
            <div class="col-sm-offset-2 col-sm-10">
			    <button type="submit" class="btn btn-default">提交操作</button>
		    </div>
        </div>
    </form>
	<div class="wl_foot"
