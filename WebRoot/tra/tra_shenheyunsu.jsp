...
<div class="page-container">
	...
	<form id="form1" name="myForm" action="examinetft" method="post">
		...
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listTransferTaskDto }" var="transferTaskDto">
                            <tr>
                                <td><input type="checkbox" name="chkItem" value="${transferTaskDto.transferTaskId }"/></td>
		...
		<div class="wl_page">
                        	...
                        	[<a disabled = "disabled">首页</a>]
                        	[<a disabled = "disabled">上一页</a>]
                        	[<a href="examinetft?pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="examinetft?pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
                        	<c:if test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a href="examinetft?pageNum=1">首页</a>]
                        	[<a href="examinetft?pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a href="examinetft?pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="examinetft?pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
