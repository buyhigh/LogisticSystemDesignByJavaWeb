...
<body>
	...
	<form id="form1" name="myForm" action="estts" method="post">
		<div class="wl_content_nav">审核派车任务</div>
		<table class="table table-striped"
			...
			<tbody>
				<c:forEach items="${requestScope.listSendTraTaskDto }"
					var="sendTraTaskDto">
					<tr>
						<td><input type="checkbox" name="chkItem"
							value="${sendTraTaskDto.sendTraTaskId }" /></td>
		...
		<div class="wl_page">
			...
			<c:if test="${requestScope.maxPage > 1 && requestScope.curPage == 1}">
                        	...
                        	[<a disabled="disabled">上一页</a>]
                        	[<a
					href="estts?pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="estts?pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
			<c:if
				test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a href="estts?pageNum=1">首页</a>]
                        	[<a
					href="estts?pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a
					href="estts?pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a
					href="estts?pageNum=${requestScope.maxPage }">末页</a>]
