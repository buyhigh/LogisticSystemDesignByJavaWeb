            ...
            <div class="wl_right">
            	<form id="form1" name="myForm" action="querytra" method="post">
                        ...
                        <div class="wl_page">
                        	...
                        	[<a disabled = "disabled">首页</a>]
                        	[<a disabled = "disabled">上一页</a>]
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
                        	<c:if test="${requestScope.maxPage > 1 && requestScope.curPage > 1 && requestScope.curPage < requestScope.maxPage }">
                        	共有<span>${requestScope.count }</span>条，分为<span>${requestScope.maxPage }</span>页，每页<span>10</span>条，当前显示第<span>${requestScope.curPage }</span>页 
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=1">首页</a>]
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=${requestScope.curPage - 1 }">上一页</a>]
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=${requestScope.curPage + 1 }">下一页</a>]
                        	[<a href="querytra?curState=${requestScope.curState }&pageNum=${requestScope.maxPage }">末页</a>]
                        	</c:if>
