...
<body>
	...
	<form id="form1" name="myForm" action="querydispatch" method="post">
		<input type="hidden" name="operate" value="" /> <input type="hidden"
			name="curState" value="${requestScope.curState.stateId }" />
		<div class="wl_content_nav">查询配送通知单</div>
		<table class="wl_table_query" cellspacing="0">
			<tr class="first">
				<th width="100">配送通知单状态</th>
				...
				<td width="440"><input type="button" value="查 询" id="btnQuery" /></td>
			</tr>
		</table>
				...
				<c:forEach items="${requestScope.listDispatchDto }"
					...
					<tr>
						...
						<td><a
							...
							<span class="wl_detailsty"
							onclick="showDetail(${dispatchDto.dispatchId})">详情...</span></td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="6" class="lastTd"><input type="button"
						value="全部选择" id="btnSelAll" /> <input type="button" value="全不选择"
						id="btnSelNot" /> <input type="button" value="删除所选" id="btnDel" /></td>
				</tr>
			</tbody>
		</table>
		<div class="wl_page">
	...
	<script type="text/javascript">
    	...
    	$("btnQuery").onclick = function(){
    		operate.value = "1";//标识查询操作
    		myForm.submit();
    	};
    	$("btnDel").onclick = function(){
    		operate.value = "2";//标识删除操作
    		myForm.submit();
    	};
